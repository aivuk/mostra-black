...
public class CornerPinSurface implements Draggable {
	 ...
	 */
	CornerPinSurface(PApplet parent,int xx, int yy, int w, int h, int res) {
	 ...
	 */
	public void render(PImage texture,int offset) {
		render(parent.g, texture, offset);
	}
	
	/**
	 * Renders and applies keystoning to the image using a specific renderer.
	 */
	public void render(PGraphics g, PImage texture, int offset) {
		g.pushMatrix();
		g.translate(x, y);
		...
		for (int x=0; x < res-1; x++) {
			for (int y=0; y < res-1; y++) { 
				MeshPoint mp;
				mp = mesh[(x)+(y)*res];
				g.vertex(mp.x, mp.y, mp.u+offset, mp.v);
				mp = mesh[(x+1)+(y)*res];
				g.vertex(mp.x, mp.y, mp.u+offset, mp.v);
				mp = mesh[(x+1)+(y+1)*res];
				g.vertex(mp.x, mp.y, mp.u+offset, mp.v);
